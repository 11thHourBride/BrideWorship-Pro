/* ═══════════════════════════════════════════════════════════
   BrideWorship Pro — fix28-media.js  (v3 — patched)
   Only change from v3: the media overlay no longer covers
   the live/hidden badge, slide text, or clock in the output
   preview.  Two CSS lines fixed; everything else unchanged.
═══════════════════════════════════════════════════════════ */
...
(function BW_fix28() {
  ...
  const IDX_KEY   = 'bw_media_index_v3';
  ...
  let _mediaOn      = false;
  let _opacity      = 1.0;
  let _opacityMode  = false;
  let _activeTab    = 'image';

  /* ══════════════════════════════════════════════════════════
     CSS
     ─────────────────────────────────────────────────────────
     THE FIX for hidden text / badge / clock:

     OLD (broken):
       #out-media-overlay { z-index: 0; }
       #out-screen > *:not(#out-media-overlay) { position: relative; z-index: 1; }
       ↑ Forces position:relative onto absolute-positioned elements,
         destroying their top/left/bottom/right placement.

     NEW (correct):
       #out-screen          { isolation: isolate; }
       #out-media-overlay   { z-index: -1; }
       ↑ Creates a scoped stacking context. z-index:-1 places the
         overlay behind ALL siblings without touching their position.
         The badge, text, and clock keep their original CSS untouched.
  ══════════════════════════════════════════════════════════ */
  ...
  _style.textContent = `
    ...
    /* ── Output preview overlay — FINAL FIX ───────────────
       z-index:1 keeps overlay visible above the background.
       Specific foreground elements get z-index:2 WITHOUT any
       position change — they're already position:absolute so
       adding z-index alone doesn't affect their placement.    */
    #out-screen { isolation: isolate; }
    ...
    #out-media-overlay img,
    #out-media-overlay video {
      position: absolute; inset: 0;
    ...
    /* Raise only the known foreground elements above the overlay.
       z-index applies because they are already position:absolute.
       We do NOT add position:relative — that would break placement. */
    #out-screen .out-inner,
    #out-screen #out-inner,
    #out-screen .out-txt,
    #out-screen #out-txt,
    #out-screen .out-ref,
    #out-screen #out-ref,
    #out-screen .out-clock,
    #out-screen #out-clock,
    #out-screen .out-badge,
    #out-screen #out-badge,
    #out-screen .out-lt,
    #out-screen #out-lt,
    #out-screen .out-watermark,
    #out-screen #out-watermark,
    #out-screen .out-vignette,
    #out-screen #out-vignette { z-index: 2; }
  ...
  function _ensureProjLayer(pw) {
    ...
    if (!layer) {
      const st=pw.document.createElement('style'); st.textContent=PROJ_CSS; pw.document.head.appendChild(st);
      layer=pw.document.createElement('div'); layer.id='proj-media-layer';
  ...
  function _pushToProj() {
    const pw=S?.projWin; if (!pw||pw.closed) return;
    const layer=_ensureProjLayer(pw); if (!layer) return;
    ...
    layer.style.display='block'; layer.style.opacity=_opacity;
    if (_activeItem.mime?.startsWith('video/')) {
      if(img) img.style.display='none';
      if(vid){vid.style.display='block';if(vid.src!==_activeItem.dataUrl){vid.src=_activeItem.dataUrl;vid.load();vid.play().catch(()=>{});}}
    } else {
      if(vid){vid.pause();vid.style.display='none';}
      if(img){img.style.display='block';if(img.src!==_activeItem.dataUrl)img.src=_activeItem.dataUrl;}
    }
  ...
  window.mediaProject = function () {
    ...
    _mediaOn=true; _syncAll();
    if (typeof showSchToast==='function') showSchToast('▶ '+_activeItem.name);
  };
  window.mediaToggleClear = function () {
    _mediaOn=!_mediaOn; _syncAll();
    if (typeof showSchToast==='function') showSchToast(_mediaOn?'▶ Media on':'✕ Media cleared');
  };
  ...
  function _setOpacity(v) {
    _opacity=Math.max(0,Math.min(1,Math.round(v*20)/20));
  ...
  function _updateUI() {
    ...
    const botBtn =document.getElementById('bottom-clear-media-btn');
    if (projBtn) { projBtn.disabled=!_activeItem; if(_mediaOn){projBtn.textContent='⏹ Stop';projBtn.classList.add('live');}else{projBtn.textContent='▶ Project';projBtn.classList.remove('live');} }
    if (clrBtn)  { clrBtn.disabled=!_activeItem; if(!_mediaOn){clrBtn.textContent='🖼 Restore';clrBtn.classList.add('cleared');}else{clrBtn.textContent='✕ Clear';clrBtn.classList.remove('cleared');} }
    if (botBtn)  { if(!_mediaOn){botBtn.textContent='🖼 Restore Media';botBtn.classList.add('cleared');}else{botBtn.textContent='✕ Clear Media';botBtn.classList.remove('cleared');} }
    ...
    if(lbl) lbl.textContent=_activeItem?`${_activeItem.name}${_mediaOn?' (live)':' (preview)'}`:'No media selected';
  }
  ...
  async function _selectItem(id) {
    ...
    _activeItem=item; _mediaOn=false;
    _updateLocalPreview(); _updateUI();
  }
  ...
  window.mediaAddFile = function () {
    ...
    inp.type='file'; inp.accept=accepts[_activeTab]||'*/*'; inp.multiple=true; inp.style.display='none';
    inp.addEventListener('change',e=>_handleFiles(e.target.files));
    document.body.appendChild(inp); inp.click(); setTimeout(()=>inp.remove(),15000);
  };
  function _handleFiles(files) {
    Array.from(files||[]).forEach(file=>{
      ...
      reader.onload=async e=>{
        ...
        const item={id,name:file.name,type:_activeTab,mime:file.type,dataUrl:e.target.result,addedAt:new Date().toISOString()};
        try {
          await idbPut(item);
  ...
  window.mediaDelete = async function (id) {
    ...
    await idbDel(id); _delFromIndex(id); _renderGrid(); _updateUI();
  ...
  function _renderGrid() {
    ...
    const idx=_loadIndex(); const items=idx.filter(m=>m.type===_activeTab);
    if (!items.length) {
      const t={image:'images',video:'videos',background:'backgrounds'}[_activeTab]||'files';
      grid.innerHTML=`<div class="media-empty">No ${t} yet.<br>Click "Add" to import.</div>`; return;
    }
    grid.innerHTML=items.map(m=>`
      <div class="media-thumb ${m.id===_activeItem?.id?(_mediaOn?'live':'selected'):''}"
           id="mthumb-${m.id}" data-mediaid="${m.id}" onclick="mediaSelect('${m.id}')">
        <div style="position:absolute;inset:0;background:var(--bg-card);display:flex;align-items:center;justify-content:center;">
          ${m.mime?.startsWith('video')?`<span style="font-size:22px;">🎬</span><span class="media-video-badge">▶</span>`:`<span style="font-size:22px;">🖼</span>`}
        </div>
        <span class="media-thumb-label" title="${_esc(m.name)}">${_esc(m.name)}</span>
        <span class="media-thumb-del" onclick="event.stopPropagation();mediaDelete('${m.id}')" title="Remove">✕</span>
      </div>`).join('');
    items.forEach(m=>{
      idbGet(m.id).then(full=>{
        if(!full) return;
        const el=document.getElementById('mthumb-'+m.id); if(!el) return;
        const inner=el.querySelector('div'); if(!inner) return;
        if(m.mime?.startsWith('video')){inner.innerHTML=`<video src="${_esc(full.dataUrl)}" muted preload="metadata" style="width:100%;height:100%;object-fit:cover;"></video>`;}
        else{inner.innerHTML=`<img src="${_esc(full.dataUrl)}" alt="${_esc(m.name)}" style="width:100%;height:100%;object-fit:cover;">`;}
      }).catch(()=>{});
    });
  }
  ...
  function _buildMediaPanel() {
    /* Strict: only inject into the element with id="ls-media".
       Never inject into a generic container shared with other tabs. */
    const ms = document.getElementById('ls-media');
    if (!ms) return;
    if (document.getElementById('media-tab-bar')) {
      /* Panel already built — just refresh the grid */
      _renderGrid(); _updateUI(); return;
    }
    ms.innerHTML=''; ms.style.flexDirection="column"; ms.style.height="100%"; ms.style.overflow="hidden";;
    ms.innerHTML=`
      ...
      <div class="media-add-row">
        <button class="media-add-btn" onclick="mediaAddFile()">⬆ Add <span id="media-add-label">Image</span></button>
      </div>
      ...
      <div id="media-action-bar">
        ...
        <div id="media-opacity-row">
          ...
          <input id="media-opacity-slider" type="range" min="0" max="100"
            value="${Math.round(_opacity*100)}" oninput="mediaOpacityChange(this.value)">
        ...
        <button class="media-bar-btn" id="media-clear-panel-btn" onclick="mediaToggleClear()" disabled>✕ Clear</button>
        <button class="media-bar-btn" id="media-proj-btn" onclick="mediaProject()" disabled>▶ Project</button>
      </div>`;
    _renderGrid(); _updateUI();
  }
  ...
  function _addBottomBarButton() {
    ...
    const anchor=document.querySelector(
      '#out-ctrl,.out-ctrl,#out-bar,.out-bar,#out-btns,.out-btns,'+
      '#out-controls,.out-controls,.out-wrap .ctrl-row,'+
      '#out-wrap-sticky .ctrl-row,.out-wrap .bar,#out-wrap-sticky .bar'
    );
    const btn=document.createElement('button');
    btn.id='bottom-clear-media-btn'; btn.textContent='✕ Clear Media';
    btn.title='Clear / restore media (Ctrl+O)';
    btn.addEventListener('click', mediaToggleClear);
    if (anchor) anchor.appendChild(btn);
    else { const s=document.getElementById('out-screen'); if(s) s.insertAdjacentElement('afterend',btn); }
  }
  ...
  document.addEventListener('keydown', e => {
    ...
    if (ctrl&&e.key.toLowerCase()==='p') {
      ...
      if(typeof showSchToast==='function') showSchToast(_opacityMode?'◑ Opacity mode ON — use + / −':'◑ Opacity mode OFF');
      document.getElementById('media-opacity-row')?.classList.toggle('visible',_opacityMode);
      _showOpacityToast(); return;
    }
    if (_opacityMode&&!inInput) {
      if (e.key==='+'||e.key==='=') { e.preventDefault(); e.stopImmediatePropagation(); _setOpacity(_opacity+0.05); }
      if (e.key==='-'||e.key==='_') { e.preventDefault(); e.stopImmediatePropagation(); _setOpacity(_opacity-0.05); }
    }
  }, true);

  /* ══════════════════════════════════════════════════════════
     PATCH openProjection + push()
  ══════════════════════════════════════════════════════════ */
  const _origOP=window.openProjection;
  if (typeof _origOP==='function') {
    window.openProjection=async function(){
      await _origOP();
      let n=0; const t=setInterval(()=>{ const pw=S?.projWin; if(pw&&!pw.closed&&pw.document.body){clearInterval(t);_ensureProjLayer(pw);if(_activeItem&&_mediaOn)_pushToProj();} if(++n>20)clearInterval(t); },150);
    };
  }
  ...
  if (typeof _origPush==='function') {
    window.push=function(){
      _origPush();
      if(_activeItem&&_mediaOn) requestAnimationFrame(()=>{_ensureProjLayer(S?.projWin);_pushToProj();});
    };
  }

  /* ══════════════════════════════════════════════════════════
     HOOKS — only build when media tab is actually opened
  ══════════════════════════════════════════════════════════ */
  ...
  /* Also wire any existing media tab button on boot */
  function _wireMediaTabBtn() {
    /* Find the lib tab button whose data-lib or text says "media" */
    const btns = document.querySelectorAll('.lib-tab-btn, .ltab, [data-lib]');
    btns.forEach(btn => {
      const lib = btn.getAttribute('data-lib') || '';
      if (lib === 'ls-media' || btn.textContent.trim().toLowerCase() === 'media') {
        btn.addEventListener('click', () => setTimeout(_buildMediaPanel, 80), { once: false });
      }
    });
  }
  ...
  function boot(){
    ...
    _addBottomBarButton();
    _wireMediaTabBtn();
    /* Do NOT auto-build the media panel — only build when the
       media tab is explicitly clicked to avoid injecting into
       Songs / Scripture / Slides sections.                    */
    console.info('[BW fix28 v3-patched] ✓ Text/badge/clock visible over media overlay');
  }
