...
(function BW_Fix3() {
  ...
  window.rwStart = async function () {
    ...
    _rwPeer.on('connection', conn => {
      ...
      conn.on('data',  d  => { if (d?.cmd) _handleRemoteCmd(d.cmd, conn); });
  ...
  function _handleRemoteCmd(cmd, conn) {
    const fns = { next:'nextSlide', prev:'prevSlide', blank:'toggleBlank', live:'toggleLive',
                  freeze:'toggleFreeze', logo:'toggleLogo', timer_start:'startTimer',
                  timer_pause:'pauseTimer', timer_reset:'resetTimer' };
    if (cmd === 'ping') { conn.open && conn.send({type:'pong'}); }
    else if (fns[cmd] && typeof window[fns[cmd]]==='function') window[fns[cmd]]();
    _broadcastState();
  }
